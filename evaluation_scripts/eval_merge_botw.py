...
from evaluation_scripts.eval_visual import bow_loader, vgt_loader, plcd_loader, sequences

def botw_loader(dataset, seq):
    botw_path = "/home/cyj/code/BoTW-LCD-master/matlab/results"
...
def merge_predictor(visual_data, pose_data, qmin, qmax, dataset, vo_name, visual_methods):
    ...
    for seq in sequences[dataset]:
        ...
        v_score = interp2d(visual_data[dataset, seq, "botw"][None], hw[-1])[0]
        ...
        predict = {
            f"{vo_name}_botw": [torch.masked_select(merge_score, gt_visual_mask), "vgt", (n_discard, n_total, ratio, qmin, qmax)]
        }
...
def metrics_and_save(pred, label, threshold, eval_out_folder, qmin, qmax):
    ...
    for method, values in pred[seq_name].items():
        ...
        # pr_filename = os.path.join(eval_out_folder, f"pr_all_{method}_{threshold:.1f}.txt")
        # np.savetxt(pr_filename, np.stack([precision, recall], 1))
...
def main(args):
    ...
    for dataname, voname in datasets_info:
        eval_out_folder = os.path.join("./train_log", args.name, "Merge_evals_BoTW", f"{dataname}_{voname}")
