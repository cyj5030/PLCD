...
def merge_predictor(visual_data, pose_data, qmin, qmax, dataset, vo_name, visual_methods):
    ...
    for seq in sequences[dataset]:
        ...
        predict = {
            f"{vo_name}_botw": [
                (merge_score * gt_visual_mask).max(-1)[0], 
                "vgt", 
                (n_discard, n_total, ratio, qmin, qmax),
    ...
    for seq_name in pred.keys():
        for method, values in pred[seq_name].items():
            ...
            if plot:
                ...
                vis(th_result, traj_info[dataname,seq_name,method[:-5]][1], save_filename)
...
def main(args):
    ...
    for dataname, voname in datasets_info:
        save_floder = os.path.join("train_log", args.name, "plots", "traj_botw_plcd", dataname, voname)
        mkdir(save_floder)
